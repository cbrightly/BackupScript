...
tar -czf $DATE-$HOST-www_backup.tar.gz /var/www;
echo "|		! /var/www made into a tarball in $DEST";
tar -czf $DATE-$HOST-etc_backup.tar.gz /etc;
echo "|		! /etc made into a tarball in $DEST";
 tar -czf $DATE-$HOST-var_lib_dpkg.tar.gz /var/lib/dpkg;
echo "|		! /var/lib/dpkg tarballed in $DEST";
 tar -czf $DATE-$HOST-apt_extended_states.tar.gz /var/lib/apt/extended_states;
echo "|		! /var/lib/apt/extended_states backed up";
 dpkg --get-selections "*" > $DEST/$DATE-$HOST-dpkg_get-selections.txt;
 chown $USER $DEST/$DATE-$HOST-dpkg_get-selections.txt;
...
echo "|		! dpkg --get-selections \"*\" backed up";
 tar -czf $DATE-$HOST-pkgstates.tar.gz /var/lib/aptitude/pkgstates;
echo "|		! aptitude package states backed up";
 tar -czf $DATE-$HOST-backup.tar.gz tar -czf $DATE-$HOST-*.tar.gz --exclude=$DATE-$HOST-backup.*;
